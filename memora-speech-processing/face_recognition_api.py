...
# Ensure this file's directory is on sys.path so sibling modules (speech_processor, etc.) resolve
...
try:
    ...
    print("[WARN] face_recognition not installed — face detection endpoints will return errors.")
...
THRESHOLD = 0.65          # Euclidean distance; raise to 0.65 for more lenient matching across real-world photos
...
def _get_embedding(img_array: np.ndarray) -> Optional[list]:
    # HOG with 1x upsampling handles phone selfie photos reliably and much faster
    locations = face_recognition.face_locations(img_array, number_of_times_to_upsample=1, model="hog")
    if not locations:
        print("[embed] No face detected (hog+upsample).")
        return None
    encodings = face_recognition.face_encodings(img_array, locations, num_jitters=1)
...
def reload_embeddings_endpoint():
    """Reload all face embeddings from disk into memory cache. Call after adding/updating profiles."""
    _embeddings_cache.clear()
    _init_embeddings()
    return {"reloaded": True, "users_loaded": len(_embeddings_cache)}
...
def iot_capture(
    ...
    # HOG with 1x upsampling — reliable for phone camera photos without CUDA
    locations = face_recognition.face_locations(img_array, number_of_times_to_upsample=1, model="hog")
    if not locations:
        print("[iot/capture] No face detected in image.")
...
# ── ESP32-CAM raw upload endpoint ─────────────────────────────────────────────
@app.post("/upload-image")
async def upload_image(request: Request, session_id: Optional[str] = None):
    """
    Receives a raw JPEG body from the ESP32-CAM board.
    The board POSTs with Content-Type: image/jpeg (no multipart).
    Runs face recognition and returns the same JSON schema as /iot/capture
    so the ESP32 can forward it straight to Flutter.
    """
    image_bytes = await request.body()
    if not image_bytes:
        return JSONResponse(
            status_code=400,
            content={"status": "error", "message": "Empty image body"}
        )

    print(f"[upload-image] session={session_id} received {len(image_bytes)} bytes")

    img_array = _load_image_from_bytes(image_bytes)
    locations = face_recognition.face_locations(img_array, number_of_times_to_upsample=1, model="hog")

    if not locations:
        print("[upload-image] No face detected.")
        return {
            "device_id": "esp32-cam",
    ...
    for encoding in encodings:
        ...
        if match and dist < THRESHOLD:
            ...
            if face_id in db_data:
                db_data[face_id]["lastSeenAt"] = now
                db_data[face_id]["lastSeenDevice"] = "esp32-cam"
...
async def update_face(request: Request, face_id: str):
    ...
    # ── Refresh embedding cache for this face ────────────────────────────────
    # If this face was previously unknown and just got a name, we must ensure
    # its embedding stays current so future captures recognise it correctly.
    photo_path = os.path.join(USERS_DIR, face_id, "profile.jpg")
    if os.path.exists(photo_path) and face_id not in _embeddings_cache:
        try:
            img_array = face_recognition.load_image_file(photo_path)
            encs = face_recognition.face_encodings(img_array)
            if encs:
                _embeddings_cache[face_id] = encs[0]
                print(f"[update_face] Refreshed embedding for {face_id}")
        except Exception as emb_err:
            print(f"[update_face] Could not refresh embedding for {face_id}: {emb_err}")
...
def delete_face(face_id: str):
    ...
    db_data.pop(face_id, None)
    _save_db(db_data)
    
    if face_id in _embeddings_cache:
        _embeddings_cache.pop(face_id, None)
    ...
    try:
        ...
        if full_text.strip():
            try:
                ...
                print("[diarize] Running keyword extraction...")
                extractor = KeywordExtractor()
                kw_data = extractor.extract_all(full_text, top_n=10)
                results["keywords"] = kw_data
                print("[diarize] Keyword extraction done.")
            except Exception as kw_err:
                print(f"[diarize] Keyword extraction failed: {kw_err}")
                results["keywords"] = {}
        else:
            results["keywords"] = {}

        # ── AI Memory Summary (Llama → rule-based fallback) ───────────────────
        if full_text.strip():
            ai_sum = None
            # 1. Try fine-tuned Llama first
            try:
                from llm_processor_finetuned import LLMProcessor
                print("[diarize] Generating AI memory summary using fine-tuned Llama model...")
                llm = LLMProcessor()
                ...
                print("[diarize] AI memory summary generated (Llama).")
            except Exception as llm_err:
                # 2. Fallback to rule-based processor (pure Python, no ML needed)
                print(f"[diarize] Llama unavailable ({llm_err}), using rule-based summary...")
                try:
                    from llm_processor_rulebased import LLMProcessor as RuleProcessor
                    llm_r = RuleProcessor()
                    ai_sum = llm_r.generate_memory_summary(
                        transcription=results,
                        keywords=results.get("keywords", {}),
                        summary={}
                    )
                    print("[diarize] AI memory summary generated (rule-based).")
                except Exception as rb_err:
                    print(f"[diarize] Rule-based summary also failed: {rb_err}")
                    ai_sum = None
